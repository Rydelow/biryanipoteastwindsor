...
if(!isset($all_options['exwoofood_disable_exoptions']) || $all_options['exwoofood_disable_exoptions']!='yes' ){
...
include plugin_dir_path(__FILE__).'shortcodes/woo-food-menu-group.php';
include plugin_dir_path(__FILE__).'shortcodes/woo-food-orderbt.php';
//widget
...
if(isset($all_options['exwoofood_foodby_odmt']) && $all_options['exwoofood_foodby_odmt']=='yes'){
	include plugin_dir_path(__FILE__).'food-by-odmt.php';
}
if(isset($all_options['exwoofood_enable_loc']) && $all_options['exwoofood_enable_loc']=='yes'){
	include plugin_dir_path(__FILE__).'food-by-loc.php';
}
include plugin_dir_path(__FILE__).'minmax-quantity.php';
// Radius shipping
include plugin_dir_path(__FILE__).'shipping.php';
// Tipping 
include plugin_dir_path(__FILE__).'tip.php';
include plugin_dir_path(__FILE__).'coupon-restrict.php';
// Order on whatsapp 
include plugin_dir_path(__FILE__).'order-whatsapp.php';
	...
	function exwoofood_template_plugin($pageName,$shortcode=false){
		if(isset($shortcode) && $shortcode== true){
			if (locate_template('woocommerce-food/content-shortcodes/content-' . $pageName . '.php') != '') {
				...
				$temp_url = exwoof_get_plugin_url().'templates/content-shortcodes/' . $pageName . '.php';
				$temp_url = apply_filters('exwf_default_sctemplate_locate',$temp_url,$pageName);
				include $temp_url;
			}
		}else{
			if (locate_template('woocommerce-food/' . $pageName . '.php') != '') {
				...
				$temp_url = exwoof_get_plugin_url().'templates/' . $pageName . '.php';
				$temp_url = apply_filters('exwf_default_template_locate',$temp_url,$pageName,$shortcode);
				include $temp_url;
			}
		}
	}
...
if(!function_exists('exwoofood_query')){
    function exwoofood_query($posttype, $count, $order, $orderby, $cat, $tag, $taxonomy, $meta_key, $ids, $meta_value=false,$page=false,$mult=false,$active_filter=false,$feature=false, $sloc = false){
		...
		return apply_filters( 'exwoofood_query', $args,$cat );
	}
}

if(!function_exists('exwf_remove_items_outofstock')){
	function exwf_remove_items_outofstock($args){
...
add_filter( 'exwoofood_query', 'exwf_remove_items_outofstock' );
add_filter( 'exwf_ajax_query_args', 'exwf_remove_items_outofstock' );
add_filter( 'exwf_ajax_filter_query_args', 'exwf_remove_items_outofstock' );
...
function ajax_exwoofood_loadmore(){
	...
	if(($orderby =='rand' || $meta_key=="_price" ) && is_array($param_ids)){
	...
	if($the_query->have_posts()){
		...
		if(is_array($param_ids) && ($page_navi=='loadmore' || $orderby=='rand')){
			?>
        ...
        <?php
        do_action('exwf_after_content_loadmore_html');
	}
	$html = ob_get_clean();
...
// register sesion
add_action('woocommerce_init', 'exwf_wc_session_user');
...
function ajax_exfood_menuegory(){
	...
	$param_query = apply_filters('exwf_ajax_filter_query_args',$param_query,$atts_cr);
	...
	if($html==''){
		...
		if($term->description!=''){
			$html_dcat ='<div class="exwf-dcat" style="display:block;">'.$term->description.'</div>';
		}
	}
	ob_start();
	// global $modal_html;
...
if(!function_exists('exwoofood_search_form_html')){
	function exwoofood_search_form_html($cats, $order_cat, $pos = false,$active_filter=false,$filter_style=false,$hide_ftall=false,$show_count=false){
		$args = array(
			'hide_empty'        => true,
			'parent'        => '0',
		);
		...
		$menucats = function_exists('exwf_menuby_cats_included') ? exwf_menuby_cats_included() : '';
		...
		$css_class = isset($filter_style) && $filter_style=='icon' ? 'exwf-fticon-style' :'';
		$mobile_filter = exwoofood_get_option('exwoofood_mb_filter');
		if($mobile_filter == 'slider'){
			$css_class .= ' exwf-filter-slider'; 
		}
		$active_filter = isset($active_filter) ? $active_filter :'';
		?>
        <div class="exfd-filter <?php echo esc_attr($css_class); ?>">
	            		...
	            		if(isset($hide_ftall) && $hide_ftall!='yes'){?>
	            			...
	            			foreach ( $terms as $term ) {
	            				if(is_array($exclude) && !empty($exclude) && in_array($term->slug, $exclude)){
		            				...
		            				$all_atcl = '';
		            				global $exwf_child_atv;
		            				$exwf_child_atv = false;
		            				global $wp;
									$curent_url = home_url( $wp->request );
									$curent_url = apply_filters('exwf_current_link',$curent_url);
									ob_start();
									exfd_show_child_inline($cats,$term,$count_stop,$order_cat,'inline',$exclude,$active_filter,$show_count);
									if(isset($active_filter) && $active_filter == $term->slug || $exwf_child_atv==true){
		            					$all_atcl = $act_cls;
		            				}
									$child_c = ob_get_contents();
									ob_end_clean();
							  		echo '<a class="ex-menu-item '.esc_attr($all_atcl).' '.($child_c!='' ? ' exwf-has-child' :'').'" href="'.esc_url(add_query_arg( array('menu' => $term->slug), $curent_url )).'" data-value="'. esc_attr($term->slug) .'">';
										...
										if(isset($show_count) && $show_count=='yes'){
											echo '<span class="exwf-cat-count"> ('.$term->count.')</span>'; 
										}
							  			echo $child_c;
							  		echo '</a>';
							  	}
		            			...
		            			foreach ( $terms as $term ) {
	            					...
	            					// if exclude
				        				...
				        				if(isset($show_count) && $show_count=='yes'){
								  		...
								  		echo '<option value="'. esc_attr($term->slug) .'" '.esc_attr($selected).'>'. wp_kses_post($term->name) .$t_count.'</option>';
								  		echo exfd_show_child_inline($cats,$term,$count_stop,$order_cat,'',$exclude,$active_filter,$show_count);
								  	}
							  	}
			                	?>
			                </select>
...
if(!function_exists('exfd_show_child_inline')){
	function exfd_show_child_inline($cats,$term,$count_stop,$order_cat,$inline,$exclude=false,$active_filter=false,$show_count=false){
		global $exwf_child_atv;
		if ($count_stop < 2) {
			return;
		}
		...
		$show_count = apply_filters('exwf_show_count_child_cat',$show_count);
			...
			if ($inline != 'inline') {
				foreach ($second_level_terms as $second_level_term) {
					...
					// if exclude
						...
						echo '<option value="'. esc_attr($second_level_term->slug) .'" '.esc_attr($selected).'>'.wp_kses_post($charactor. $second_level_term->name) .$t_count.'</option>';
						exfd_show_child_inline($cats,$second_level_term,$count_stop,$order_cat,'','',$active_filter,$show_count);
					}
				}
			}else{
		        ...
		        foreach ($second_level_terms as $second_level_term) {
					...
					// if exclude
			            ...
			            $atcl = '';
        				if(isset($active_filter) && $active_filter == $second_level_term->slug){
        					$atcl = 'ex-menu-item-active';
        					$exwf_child_atv = true;
        				}
						$t_count ='';
        				if(isset($show_count) && $show_count=='yes'){
							$t_count = '<span class="exwf-cat-count"> ('.$second_level_term->count.')</span>'; 
						}				
			            echo '<li class="exfd-child-click ex-menu-item '.esc_attr($atcl).'" data-value="'.esc_attr($second_level_term->slug).'" data-url="'.esc_url(add_query_arg( array('menu' => $second_level_term->slug), $curent_url )).'">
			            '.wp_kses_post($second_term_name).$t_count;
			            exfd_show_child_inline($cats,$second_level_term,$count_stop,$order_cat,'inline','',$active_filter,$show_count);
			            echo '</li>';
			        }
		        }
...
if(!function_exists('exwoofood_sale_badge')){
	function exwoofood_sale_badge(){
		...
		if ( is_object($product) && method_exists($product, 'is_on_sale') && $product->is_on_sale() ) { ?>
			<div class="exfd-ribbon exwf-sale-bg"><span><?php esc_html_e('Sale','woocommerce-food');?></span></div>
			<?php 
		}elseif(exfd_show_reviews('',$product)!=''){
			echo '<div class="exfd-ribbon exwf-review-bg"><span>'.exfd_show_reviews('',$product).'</span></div>';
		}
	}
}
...
if(!function_exists('exwoofood_add_to_cart_form_shortcode')){
	function exwoofood_add_to_cart_form_shortcode( $atts ) {
		global $cart_itemkey;
		$cart_itemkey = isset($atts['exwf_cart_itemkey']) ? $atts['exwf_cart_itemkey'] : '';
...
function ajax_exwoofood_booking_info(){
	if(isset($_POST['id_food']) && $_POST['id_food']!=''){
		...
		global $atts,$id_food,$param_shortcode,$exwf_itemkey;
		$atts['exwf_cart_itemkey'] = isset($_POST['cart_itemkey']) ? $_POST['cart_itemkey'] : '';
		$param_shortcode = isset($_POST['param_shortcode']) ? $_POST['param_shortcode'] : '';
...
function exwf_get_error_notices(){
	return array( 'error');
}
...
function exwoofood_ajax_add_to_cart() {
    ...
    $passed_minmax = apply_filters('exwf_add_to_cart_validation_minmax', true, $product_id, $quantity,$variation_id,'');
    if($passed_minmax==false){
    	$data = array(
            'error' => true,
            'message' => '<p class="exfd-out-notice">'.wc_print_notices( true).'</p>'
        );
        wc_clear_notices();
    ...
    if ($passed_validation || $un_vali == true) {
        ...
        $data = array(
            'error' => true,
            'message' => '<div class="exfd-out-notice">'.wc_print_notices( true).'</div>'
        );
        wc_clear_notices();
        echo wp_send_json($data);
    }*/
    add_filter('woocommerce_notice_types', 'exwf_get_error_notices');
...
if(!function_exists('exwoofood_booking_button_html')){
	function exwoofood_booking_button_html($style,$hide_atc=false) {
		if (!exwf_check_open_close_time(get_the_ID()) || (isset($hide_atc) && $hide_atc=='yes' ) ) {
			return;
		}
		$order_text = apply_filters('exwf_order_button_text',esc_html__( 'Order', 'woocommerce-food' ));
		$html = '<a href="'.get_the_permalink(get_the_ID()).'" class="exstyle-'.esc_attr($style).'-button">'.$order_text.'</a>';
    	...
    	if($product!==false) {
        	...
        	$disable_atc = apply_filters( 'exwf_disable_add_to_cart', 'no' );
        	if($type =='variable' || $disable_atc =='yes'){
...
function exwoofood_select_loc_html($atts){
	...
	ob_start();
	$loc_selected = isset(WC()->session) ? WC()->session->get( 'ex_userloc' ) : '';
	?>
...
if(!function_exists('exfd_show_child_location')){
	function exfd_show_child_location($locations,$term,$count_stop,$loc_selected,$checkout,$s_all=false){
		if ($count_stop < 2) {
			return;
		}
		$charactor ='';
		...
		$args_child = apply_filters('exwf_location_args',$args_child);
		if(isset($s_all) && $s_all==true){$args_child['exclude']='';}else{ $s_all='';}
		...
		if ($second_level_terms) {
			...
			foreach ($second_level_terms as $second_level_term) {
				...
				exfd_show_child_location($locations,$second_level_term,$count_stop,$loc_selected,$checkout,$s_all);
			}
	    }
	    $output_string = ob_get_contents();
		ob_end_clean();
...
if(!function_exists('exwf_check_open_close_time')){
	function exwf_check_open_close_time($id_cr=false){
		...
		$closed_dates = exwoofood_get_option('exwfood_opcl_datetodate','exwoofood_advanced_options');
...
if(!function_exists('exwfd_open_closing_message')){
	function exwfd_open_closing_message($rhtml=false){
		...
		if($enable_time=='closed' || $closed_loc == 'yes'){
			...
			$text = sprintf( esc_html__( 'Ordering food is now closed, please come back after %1$s %2$s', 'woocommerce-food' ), $date_cls, $time_cls);
		...
		if(isset($rhtml) && $rhtml==true ){
			...
			echo '<p class="exfd-out-notice excls-notice">' .$text.'</p>';
		}
		$html = ob_get_contents();
		ob_end_clean();
		return $html;
	...
	if(is_array($icons) && !empty($icons)){
		...
		foreach ($icons as $icon) {
			...
			<span>
				...
				<?php echo $lb!='' ? '<span class="exwf-lbname">'.$lb.'</span>' : '' ?>
			</span>
			<?php
		}
		echo '</span>';
...
*/
add_action( 'template_redirect', 'exwfredirect_single_product' );
function exwfredirect_single_product() {
	$menu_url = exwoofood_get_option('exwoofood_menu_url','exwoofood_options');
	if ($menu_url !=''&& is_singular( 'product' ) &&  filter_var($menu_url, FILTER_VALIDATE_URL) !== FALSE ) {
		...
		$menu_url = apply_filters('exwf_single_url_popup',$menu_url,$postid);
		wp_redirect( $menu_url);
		exit;
	}
}
add_action( 'exwoofood_before_shortcode', 'exwf_add_hidden_product_from_link' );
function exwf_add_hidden_product_from_link() {
	if (isset($_GET['wfproduct_id']) && is_numeric($_GET['wfproduct_id'])) {
		remove_action( 'exwoofood_before_shortcode', 'exwf_add_hidden_product_from_link' );
		$id_el = 'exwf-single-'.$_GET['wfproduct_id'];
		echo '<div class="ex-hidden" id="'.esc_attr($id_el).'">'.do_shortcode('[ex_wf_ordbutton product_id="'.$_GET['wfproduct_id'].'"]').'
			<script>
			jQuery(document).ready(function($) {
				var contenId = "#'.esc_attr($id_el).' .item-grid[data-id_food='.$_GET['wfproduct_id'].'] a";
				if($(contenId).length){
